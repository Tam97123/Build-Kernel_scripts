...
#     if ! curl -LO https://raw.githubusercontent.com/Tam97123/Build-Kernel_scripts/refs/heads/main/scripts/integrate_ksu.sh; then
#      echo "Error: Can not download script."
#      exit 1
#     fi
#     chmod +x integrate_ksu.sh && ./integrate_ksu.sh
#    else
#     echo "Already patch KernelSU into kernel source"
#    fi
#}

intergrate_ksu_susfs () {
    if [ ! -f "integrate_ksu_susfs.sh" ]; then
     echo "Downloading script..."
     if ! curl -LO https://raw.githubusercontent.com/Tam97123/Build-Kernel_scripts/refs/heads/main/scripts/integrate_ksu_susfs.sh; then
      echo "Error: Can not download script."
      exit 1
     fi
     chmod +x integrate_ksu_susfs.sh && ./integrate_ksu_susfs.sh
    else
     echo "Already patch KernelSU with SUSFS into kernel source"
    fi
    echo "Downloading defconfig to enable KSU..."
    if ! curl -L https://github.com/Tam97123/Build-Kernel_scripts/raw/refs/heads/main/defconfig/ksu-susfs_defconfig -o $DEFCONFIG_DIR/ksu-susfs_defconfig; then
     echo "Error: Can not download file."
     exit 1
    fi
    CUSTOM_DEFCONFIG="$CUSTOM_DEFCONFIG ksu-susfs_defconfig"
}
